...
void HDF5DataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < batch_size; ++i, ++current_row_) {
    ...
    for (int j = 0; j < this->layer_param_.top_size(); ++j) {
      ...
      //LOG(INFO)<<"top[j]->count(): "<<top[j]->count()<<" top[j]->shape(0): "<<top[j]->shape(0);
      caffe_copy(data_dim, //copy data to memory(copy starting address)
          &hdf_blobs_[j]->cpu_data()[data_permutation_[current_row_]
            * data_dim], &top[j]->mutable_cpu_data()[i * data_dim]);
//      //debug
//      if(j==1){
//    	  LOG(INFO)<<"row: "<<current_row_<<" address:  "<<&hdf_blobs_[j]->cpu_data()[data_permutation_[current_row_]* data_dim]
//					<<" data:  "<<hdf_blobs_[j]->cpu_data()[data_permutation_[current_row_]* data_dim];
//      }
    }
  }
}
