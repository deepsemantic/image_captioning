...
void LSTMLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  hidden_param.mutable_inner_product_param()->set_num_output(num_output * 4); //"num_output * 4": for i, o, f and c gates
  ...
  //     W_xc_x = W_xc * x + b_c
  {
    ...
    x_transform_param->add_bottom("x");  //input x
    x_transform_param->add_top("W_xc_x"); // output after transformation
  }
